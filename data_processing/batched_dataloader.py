...
def load_and_process_batch(batch_csv, is_test_loader = False):

    if isinstance(batch_csv, list):
    ...
    grouped = df_data.groupby('event_id', sort=False)
    max_length_raw = grouped.size().max()
    ...
    for event_id, event_data in grouped:
        ...
        if is_test_loader:
            output_data_tensor = torch.tensor(event_data[["particle_id","weight", "pt", "eta"]].values, dtype=torch.float32)
        else:
        ...
        cluster_tensor = torch.tensor(event_data['event_id'].values)
    ...
    if not is_test_loader:
        output_data_tensors = torch.cat(output_data_tensors, dim=0)

    cluster_tensor = torch.stack(cluster_tensors)
    original_lengths_tensor = torch.tensor(lengths, dtype=torch.long)

    
    return input_data_tensor, output_data_tensors, cluster_tensor, original_lengths_tensor


def gather_batch_files(folder_path, pattern_batch="batch_*.csv",
...
class ConcatBatchDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.use_double_batches:
            batch_csv = self.pairs_list[2*idx]
            batch_csv2 = self.pairs_list[2*idx + 1]
            batch_csv = [batch_csv, batch_csv2]
        else:
            batch_csv = self.pairs_list[idx]
            
        input_data_tensor, output_data_tensor, cluster_tensor, lengths_tensor = load_and_process_batch(batch_csv)
        return input_data_tensor, output_data_tensor, cluster_tensor, lengths_tensor

class ConcatBatchDatasetTest(ConcatBatchDataset):
    def __getitem__(self, idx):
        if self.use_double_batches:
            batch_csv = self.pairs_list[2*idx]
            batch_csv2 = self.pairs_list[2*idx + 1]
        ...
        input_data_tensor, output_data_tensor, cluster_tensor, lengths_tensor = load_and_process_batch(batch_csv, is_test_loader=True)
        return input_data_tensor, output_data_tensor, cluster_tensor, lengths_tensor

def create_train_dataloaders(folder_path, use_double_batches = False):
    """
...
def create_test_dataloader(folder_path, use_double_batches = False):
    logging.info(f"Loading data from {folder_path}")
    train_pairs, val_pairs, test_pairs = gather_batch_files(folder_path)
    logging.info(f"Train has {len(train_pairs)} pairs, Val {len(val_pairs)}, Test {len(test_pairs)}")

    test_ds = ConcatBatchDatasetTest(test_pairs, use_double_batches)
...
def main():
    ...
    train_loader, val_loader = create_train_dataloaders(folder_path)
    test_loader = create_test_dataloader(folder_path)
