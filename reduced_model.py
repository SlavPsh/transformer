...
class TransformerRegressor(nn.Module):
    ...
    def __init__(self, num_encoder_layers, d_model, n_head, input_size, output_size, dim_feedforward, dropout, use_att_mask=False, wandb_logger=None, use_flash_attention=False):
        ...
        # Only map uint8 coordinate to a very low dimension
        self.layer_embedding = nn.Embedding(256, 4)  # Small embedding space
        self.input_layer = nn.Linear(input_size - 1, d_model - 4)  # Reduced d_model for float32 coordinates
    ...
    def forward(self, input_coord, input_for_mask, padding_mask):
        ...
        embedded_layer = self.layer_embedding()
        full_embedding = torch.cat([x, embedded_layer], dim=-1)
